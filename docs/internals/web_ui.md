...
- The stored-PID length cap is **derived**, not a round number: `PID_TEXT_MAX` in `main.js` = 2 (service) + the widest `MODE_IDENT_LEN` + 1 (frames hint) = 15. It was hardcoded to `10` before issue #51, which silently rejected **every** mode-23 row on Store with a message about the PID being "less than 10 characters" — a limit nothing enforced or needed. 15 is also the real firmware ceiling: `polllog_req_bytes()` stops at `POLLLOG_MAX_REQ_BYTES` (7), so a longer string would be quietly polled as *different* bytes. Don't re-hardcode it — widen `PID_MODES` and it follows.
