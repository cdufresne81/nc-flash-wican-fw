...
**Known exceptions to the byte-identical invariant:** `batt_alert` and `periodic_wakeup` are *retired*, not merely hidden — `config_server_parse_cfg_into()` force-pins both to `"disable"`, so a device whose stored config had either set to `"enable"` shows that one key flipped after a save/revert cycle. Intentional; not a regression. Per-PID `Init` → `Mode` (issue #31) is a **one-time migration**: the first Store after the update drops each pid's `"Init"` key (free-text ATSH strings the Datalogger protocol never executed) and adds `"Mode"` — always emitted and **derived from the PID text's leading service byte** (mirroring `pid_prefix_mode()` in `autopid_config.c`), so the stored key can never contradict the wire. The dropdown is only a cross-check: a hand-flipped select contradicting an expressible 01/22 prefix throws; other services (09, 23, …) warn to console and save from the prefix — they must not block the table save. After that first save the cycle is byte-stable again. A dropped non-`ATSH7E0;` Init is `console.warn`ed at load.
