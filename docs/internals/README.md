...
| [returning-to-datalogger.md](returning-to-datalogger.md) | Why a device stops recording and what brings it back: the park/claim/flash coexistence flags and when the dead-man reaper refuses, the recording gates, plus a diagnosis and recovery runbook. Also why the stored-mode failure class (#92) no longer exists and why editing the deprecated `protocol`/`port`/`port_type` keys does nothing |
| [bench-slcan-strand-2026-08.md](bench-slcan-strand-2026-08.md) | Dated snapshot (August 2026): the #92 Bench SLCAN strand -- the NC Flash probe-fallback hypothesis, what the logs do and do not prove, and the staged plan to reproduce it deterministically. **History:** #92 was closed in v1.23.0 by deleting the mode field, not by repairing the strand |
