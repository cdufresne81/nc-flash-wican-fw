...
- **Graceful-stop data loss must stay zero** (on-close flush at `csv_logger.c:285-287` intact); abrupt-power-loss window must not exceed one accumulator tail (`< 512` B).
