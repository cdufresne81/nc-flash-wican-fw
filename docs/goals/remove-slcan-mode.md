...
- The exact user-facing error an NC Flash <= v2.8.0 shows when its slcan handshake times out, and whether its error path still posts the protocol restore (tool source not on this machine; the once-per-session switch structure IS verified, per docs/goals/host-lock-ui-and-mode-lockdown.md, section "The cited evidence").
